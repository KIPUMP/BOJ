...
def count_lucky_permutations(s) :
    ...
    return generate_permutation(chars,"",used,"\0")

def generate_permutation(chars,current,used,last_char) :
    ...
    for i in range(len(chars)) :
        ...
        if len(current) > 0 and chars[i] == last_char :
            continue
        used[i] = True
        count += generate_permutation(chars,current,+chars[i],used,chars[i])
        used[i] = False
    return count
